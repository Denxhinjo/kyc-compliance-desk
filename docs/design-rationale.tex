...
\usepackage{lmodern}
\usepackage[margin=2.4cm,bottom=1.5cm]{geometry}
\usepackage{microtype}

% Document properties, so the file identifies its author when opened or
% catalogued. Producer and Creator are left to the TeX toolchain, which is
% what they are for.
\pdfinfo{
  /Author   (Denis Xhabrahimi)
  /Title    (Design rationale: KYC onboarding and compliance review desk)
  /Subject  (Architecture and design decisions, with the options rejected)
  /Keywords (KYC, AML, sanctions screening, PostgreSQL, job queue, audit log)
}
...
A sanctions screen that runs against half a list returns fewer matches and looks
like a clean result. A disclosure filter that shows an event type it does not
recognise leaks. A shared secret that is unset and therefore lets everyone
through is worse than one that locks everyone out. The failure that does not
announce itself is the expensive one, because nobody goes looking for it.
...
When a verification result arrives by webhook I must do two things: record that
it arrived, and schedule the work it implies. If they are not atomic, no ordering
is correct. Enqueue first and the database write can fail, leaving a job that
refers to a row which does not exist. Write first and the enqueue can fail,
leaving a result nobody processes --- an applicant waiting forever, no error
anywhere.
...
I rejected Redis and Celery for exactly this. Both are better queues in isolation
--- faster, with richer scheduling and visibility --- and both are a \emph{second}
system, which means a distributed transaction I would have to fake with an outbox
pattern: write to Postgres, write a row saying ``I still need to tell Redis'',
drain that row into Redis. At which point I have built a database-backed queue
anyway, plus Redis, and the database-backed queue is the one carrying
correctness. I would rather have that without the dependency in front of it.
...
of jobs per second this becomes the bottleneck and a purpose-built broker earns
its operational cost. This system handles a few hundred applications a day --- I
am nowhere near it, and the honest version of the argument names the point at
which I would change my mind.
...
no lock --- kept so the comparison is reproducible rather than rhetorical.

Five runs today, two workers against 200 jobs with no lock, gave 198, 183, 176,
192 and 198 duplicates. The third, exactly: \textbf{376 executions of 200
distinct jobs, 176 of them run twice}. That spread is wide and I would rather
print it than narrow it --- the count swings by twenty between runs on one
machine, because this is a race, and how often two workers land on the same row
is scheduling luck. The same five runs \emph{with} the lock: 200 executions, 200
distinct jobs, zero duplicates, five times out of five.

A test asserts the broken version is \emph{still broken}. That looks absurd until
you consider how it fails: someone reads the function, sees a race, fixes it, and
the comparison quietly becomes a claim about nothing.
...
declined document check'' is one. ``The model scored 0.83'' is not, and neither
is a SHAP plot.

Two consequences I chose deliberately. PEP matches cannot on their own reach
the rejection threshold: being a politically exposed person is not illegal, and
wholesale refusal --- de-risking --- is something regulators criticise, while
the law asks for enhanced due diligence, which means a human. And a sanctions
match alone routes to review rather than rejection, because screening finds
\emph{names}, and identity is a judgement a person makes.

Every scored application stores the ruleset version, the thresholds in force,
and the revision of each FATF country list it was scored against: the plenary,
its publication date, and a digest of the codes.

That last part was a correction, not a design. The lists were a hand-typed
constant, and checked against source they proved to be a set FATF never
published --- Monaco, grey-listed June 2024, beside T\"urkiye and the UAE, both
de-listed by then. FATF publishes each list complete at each plenary, so an
absence is as much a statement as a presence, and a set mixing two is wrong in
both directions: it screens against countries already cleared and misses those
added since. It is now one statement's complete set, carrying that statement's
date, with a test that fails if the codes move without the date.
...
Three reasons an applicant is not shown that, none needing a citation to stand
up. First, tipping-off: financial-crime regimes generally restrict telling a
customer what screening has happened about them, and rather than work out where
the line falls in each jurisdiction, the safe operating rule is that screening
detail does not reach its subject --- then the legal question never arises.
Second, it teaches evasion: ``matched at 86\%, downgraded because the listed date
of birth conflicts'' tells someone exactly which field to change. Third, and most
often overlooked, \emph{most of these matches are wrong}: a name similarity is a
finding about a name, not a person, and showing it attaches a sanctioned
individual's identity to an applicant who is, ordinarily, somebody else.
...
One subtlety, because it is easy to undo: this works because the page is a
React Server Component, so the raw rows never cross to the browser. Moving that
list into a Client Component would serialise them into the payload again ---
leaking without a line of rendering code changing. A comment would not survive
that refactor, so a test fetches the live page and asserts that ``OFAC'', ``name
similarity'', the score and the raw payload keys appear nowhere in the response,
with a counterweight asserting the page still renders the applicant's own
history --- a disclosure assertion is trivially satisfied by a blank page.
...
\emph{``was this person screened against the list as it stood that day?''} --- was
not hard to answer, it was unanswerable.

Now every screening result points at a snapshot row recording the source, the
publisher's publication date, the record count, a SHA-256 of the file and when it
was loaded. The chain runs decision → match → list version → digest.

\textbf{What the hash proves:} that two screening runs recording the same digest
used byte-identical content, which makes ``the same list'' checkable rather than
assumed.

\textbf{What it does not prove.} The digest is of the JSON my downloader writes,
not the XML the US Treasury published, so it attests to my \emph{parser's
output} rather than the publisher's document. That file is gitignored, so a
reader cannot recompute it from the repository. And it is recorded, not
verified: nothing checks the digest on load, so a corrupted file would be
recorded faithfully as corrupt. The guard for that is a cross-check against
OFAC's declared record count and the previous snapshot, which I have not
built.

Staleness is also unsolved: the list is loaded at deployment and nothing refreshes
it. Recording the publication date makes that \emph{visible} --- an old date next
to a recent decision is readable evidence --- which is better than invisible, but
...
scanned per applicant. \textbf{Every one of those entries is loaded and
searched}, vessels and companies as well as people. A subset of 7{,}429 --- the
person-shaped names that also carry a date of birth --- appears only in the seed
script, which draws synthetic applicant names from it so that some applicants
deliberately resemble somebody listed. It narrows who gets \emph{invented},
never what anybody is screened \emph{against}.
That is fine for a process
...
a median of 224 entries and a mean of 424, roughly 2\% of the list --- and then
shortlist with exactly the code that ran before.

I indexed tokens rather than whole names, and that was the real decision.
Across the 131 matches the scorer accepts for 400 applicants, whole-string
trigram similarity falls as low as 0.194; even a pair as recognisable as
\texttt{ahmed hassan} against \texttt{ahmad hasan} scores only 0.471. The reason
is that transliteration drift lives inside words, and one extra middle name
wrecks a whole-string comparison while leaving every other token identical. The
best-matching \emph{token} in those same 131 pairs never falls below 0.357, and
its median is 1.000 --- the name part that matches usually matches exactly. A
whole-string pre-filter at 0.25 would drop one of the 131; the per-token
shortlist at the same 0.25 drops none.
...
(accepted) and 0.182 on trigram similarity (rejected).
...
is worth more than shortlist size here --- a shortlist missing the one true
match costs a sanctioned person an account. That choice is not free, and the
cost lands on the tail.

\subsection{What it costs}

Faster for almost everyone, slower for the worst case. Over 300 applicants on
one machine, by shortlist size --- 10--49 entries: 190\,ms in memory against
13\,ms two-stage; 50--199: 204 against 27; 200--999: 212 against 65; 1{,}000 and
over: 228 against 177. Worst observed, 457\,ms against 623.

The in-memory scan is flat by construction, 190 to 228\,ms, because it compares
against all 19{,}393 entries whatever the name. Two-stage spans 13 to 623\,ms, a
48$\times$ spread, because its cost is set by how many entries the pre-filter
admits: a common surname on a list full of similar spellings produced a
4{,}608-entry shortlist and a slower answer than scanning everything. So ---
typically three to eight times faster, occasionally worse, and \emph{less
predictable}. I took that trade because the alternative was never a fast
in-memory scan, it was a function that could not hold the list at all. But a
latency budget here would have to be written against the tail, and the tail is
...
A mechanism is worth what its execution is worth, though, and this one spent two
days running nowhere. The equivalence test reads the development database rather
than the throwaway one, because it needs a loaded list and a seeded population;
the CI runner provisioned neither, so the job died on a missing table and stayed
red for seven pushes while my README badge advertised it. It passed on my
machine throughout, which is why I did not see it. The runner now provisions
what the test reads, and a failed suite opens an issue --- what had failed was
not the test, but the noticing.

\section{When one service may call another}

The two services communicate only through the database, to avoid an internal
API --- one service asking the other to do something and waiting to hear how it
went, which drags CORS, shared auth, retries and a second failure mode into a
design that had none.
...
correct, only slower}. The queue carries the meaning; the call carries timing.

The test then proved itself unplanned: the call silently did nothing for a period
after deployment, and nothing broke. Applications were still decided by the
scheduled path, minutes later instead of seconds --- a latency regression rather
than a correctness one, exactly as the design predicted.

\section{The worker as an on-demand function}

Locally the worker is a long-lived loop: claim, run, sleep, repeat --- the right
shape, and what \texttt{docker compose up} gives you. It is also a process that
must run somewhere permanently, and permanently costs money. I am not paying a
monthly bill to host a portfolio demo.
...
function has a wall-clock limit, and a loop running until the queue emptied would
be killed mid-job on every invocation once a backlog existed, making no progress
while looking busy. It is called repeatedly instead: by the web app when work is
enqueued, and every five minutes by a scheduler that also runs the recurring
jobs.

The queue's guarantees are untouched, which is the part I care about. Both entry
...
Same logic, two runtimes --- rather than pretending the deployed thing is the
same thing.

\textbf{The cost:} an applicant arriving when the database has been idle waits
about 2.4 seconds for the first page that queries it, then roughly 0.3 after ---
and the five-minute schedule keeps it awake, so it is rarely paid at all.

\par\vspace{0.4em}\hrule\vspace{0.35em}\noindent
{\footnotesize Synthetic data throughout. Every figure here was measured, not estimated.}
