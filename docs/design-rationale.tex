...
\usepackage{lmodern}
\usepackage[margin=2.4cm,bottom=1.7cm]{geometry}
...
The two services communicate only through the database. That rule exists to avoid
an internal API --- one service asking the other to do something and waiting to
hear how it went, which drags CORS, shared auth, retries and a second failure
mode into a design that had none.
...
not paying a monthly bill to host a portfolio demo.
...
about 2.4 seconds for the first page that queries it, then roughly 0.3 for every
page after. The five-minute schedule keeps the database awake, so this is paid
only if the schedule has stopped.

\vspace{0.5em}
\rule{\textwidth}{0.4pt}
{\footnotesize Synthetic data throughout; no real person's data has been in this
system. Every figure was measured, not estimated; where one moves, I say so.}
